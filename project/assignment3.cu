...
__global__ void simulate(volatile int *task_schedule_status, int *priority, int *executionTime,  volatile int *priority_to_core_map, volatile int *core_free_status, volatile  int *core_busy_time, int *result, int n, int m) {
    ...
    // this will make sure all the threads run in kinda sequential manner
    while (idx > 0 && task_schedule_status[idx - 1] == 0) ;

    int p = priority[idx];
    int core_idx = priority_to_core_map[p];
    // simply get the core index if priority is already mapped to the core
    // else return -1 => we need to allocate core in next snippet
    ...
    if (core_idx == -1) {
        ...
        // now all tasks with this priority should execute on this core only
        priority_to_core_map[p] = core_idx;
    }

    // if required core is busy, we need to wait untill core is free again
    // and hence we reset the time to task's completion time on that core
    if (core_free_status[core_idx] == 1) {
        t = core_busy_time[core_idx];

        // We just updated the time and hence we need to check if other cores are free now!
        for (int i = 0; i < m; i++) {
    ...
    // t denotes the current time at which we want to execute the current task
    result[idx] = t + executionTime[idx];
    // printf("t=%d | task-%d is scheduled on core-%d untill %d units time\n", t, idx, core_idx, result[idx]);

    core_busy_time[core_idx] = result[idx]; // set time till when core would be busy
    core_free_status[core_idx] = 1; // make sure this core is busy now
